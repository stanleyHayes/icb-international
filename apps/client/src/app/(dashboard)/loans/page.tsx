            ...
            <Card>
              <CardBody className="pt-5">
                ...
                <dl className="mt-4 space-y-2 border-t border-[var(--icb-border)] pt-4 text-sm">
                  ...
                  <Row label="Instalment" value={<Amount value={loan.instalment} size="sm" />} />
                  <Row
                    label="Next payment"
                    value={loan.nextPaymentOn ? formatDate(loan.nextPaymentOn, 'medium') : 'None due'}
                  />
                  <Row
                    label="Remaining"
                ...
                {loan.arrears ? (
                  <p className="mt-4 rounded-[var(--radius-md)] border border-[var(--icb-warning-border)] bg-[var(--icb-warning-bg)] px-3.5 py-2.5 text-xs text-[var(--icb-warning-fg)]">
                    ...
                    {loan.arrears.daysPastDue} days. Bring the account up to date to avoid further
                    charges.
                  </p>
                ) : null}
              </CardBody>
          ...
          {applications.map((application) => (
            <li key={application.id}>
              <Link
                ...
                className="block px-5 py-4 transition-colors hover:bg-[var(--icb-bg-muted)]"
              >
          ...
          {products.map((product) => (
            <Card key={product.code}>
              <CardBody className="flex h-full flex-col pt-5">
                ...
                <Link
                  href={`/loans/apply?product=${product.code}`}
                  className="mt-4 inline-flex h-9 items-center justify-center rounded-[var(--radius-md)] border border-[var(--icb-border-strong)] px-4 text-sm font-medium transition-colors hover:bg-[var(--icb-bg-muted)]"
                >
                  Check my rate
                </Link>
              </CardBody>
