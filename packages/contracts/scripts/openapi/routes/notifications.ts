...
export const notificationsOperations = defineOperations([
  {
    method: 'get', path: '/notifications', tag: TAG.notifications, operationId: 'listNotifications',
  ...
  {
    method: 'post', path: '/notifications/{notificationId}/read', tag: TAG.notifications,
    operationId: 'markNotificationRead', summary: 'Mark one notification as read',
    pathParams: { notificationId: idSchema },
    response: success(STATUS.noContent, 'Marked as read.'),
    errors: [{ status: STATUS.notFound }],
  },
  {
    method: 'post', path: '/notifications/read-all', tag: TAG.notifications,
    operationId: 'markAllNotificationsRead', summary: 'Mark everything as read',
    response: success(STATUS.noContent, 'All notifications marked as read.'),
  },
  {
    method: 'get', path: '/notifications/preferences', tag: TAG.notifications,
    operationId: 'getNotificationPreferences', summary: 'The per-channel preference matrix',
    response: success(STATUS.ok, 'Preferences and quiet hours.', notificationPreferencesSchema),
  },
  {
    method: 'put', path: '/notifications/preferences', tag: TAG.notifications,
    operationId: 'updateNotificationPreferences', summary: 'Replace the preference matrix',
