...
export const adminOperations = defineOperations([
  {
    method: 'get', path: '/admin/dashboard', tag: TAG.admin, operationId: 'getAdminDashboard',
    summary: 'Live KPIs, queue depths, and volume series (staff)',
    response: success(STATUS.ok, 'The ops dashboard.', dashboardSchema),
  },
  {
    method: 'get', path: '/admin/monitor', tag: TAG.admin, operationId: 'monitorTransactions',
    summary: 'The global transaction monitor (staff)',
    query: monitorQuerySchema,
    response: success(STATUS.ok, 'An offset page of monitor entries.', PAGE_SCHEMAS.MonitorEntryPage),
  },
  {
    method: 'post', path: '/admin/postings', tag: TAG.admin, operationId: 'createManualPosting',
  ...
  {
    method: 'post', path: '/admin/transactions/{transactionId}/reverse', tag: TAG.admin,
    operationId: 'reverseTransaction', summary: 'Reverse a posted transaction (maker-checker)',
    pathParams: { transactionId: idSchema },
    request: reverseTransactionRequestSchema,
    idempotent: true,
    response: success(STATUS.accepted, 'The approval request for the reversal.', approvalRequestSchema),
    errors: [
  ...
  {
    method: 'get', path: '/admin/ledger/trial-balance', tag: TAG.admin, operationId: 'getTrialBalance',
    summary: 'The trial balance — proof the books add up (staff)',
    response: success(STATUS.ok, 'The trial balance.', trialBalanceSchema),
  },
  {
    method: 'get', path: '/admin/ledger/reconciliation', tag: TAG.admin,
    operationId: 'getReconciliation', summary: 'Reconciliation for a business date (staff)',
    query: z.object({ date: isoDateSchema }),
    response: success(STATUS.ok, 'The reconciliation view.', reconciliationSchema),
  },
  {
    method: 'get', path: '/admin/system/health', tag: TAG.admin, operationId: 'getSystemHealth',
