...
export const governanceOperations = defineOperations([
  {
    method: 'get', path: '/governance/staff', tag: TAG.governance, operationId: 'listStaffUsers',
    summary: 'Staff users and their roles (admin)',
    response: success(STATUS.ok, 'All staff users.', z.array(staffUserSchema)),
  },
  {
    method: 'post', path: '/governance/staff', tag: TAG.governance, operationId: 'createStaffUser',
  ...
  {
    method: 'get', path: '/governance/audit', tag: TAG.governance, operationId: 'searchAuditEvents',
  ...
  {
    method: 'get', path: '/governance/audit/integrity', tag: TAG.governance,
    operationId: 'verifyAuditIntegrity', summary: 'Verify the audit hash chain (staff)',
    response: success(STATUS.ok, 'The verification result.', auditIntegritySchema),
  },
  {
    method: 'get', path: '/governance/approvals', tag: TAG.governance, operationId: 'listApprovals',
    summary: 'The maker-checker inbox (staff)',
    query: offsetQuerySchema,
    response: success(STATUS.ok, 'An offset page of approval requests.', PAGE_SCHEMAS.ApprovalRequestPage),
  },
  {
    method: 'post', path: '/governance/approvals/{approvalId}/decision', tag: TAG.governance,
    operationId: 'decideApproval', summary: 'Approve or reject (four-eyes; self-approval blocked)',
